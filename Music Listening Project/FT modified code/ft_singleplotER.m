...
if strcmp(dtype, 'timelock') && hasrpt,
  ...
  for i=1:Ndata
    if isfield(varargin{i},cfg.maskparameter), tmpmsk = varargin{i}.mask; end
    varargin{i} = ft_timelockanalysis(tmpcfg, varargin{i});
    if isfield(varargin{i},cfg.maskparameter), varargin{i}.mask = tmpmsk; end
  end
...
for i=1:Ndata
  ...
  datavector_sem = reshape(mean(dat_sem, 1), [1 numel(xval)]); % average over channels
  ...
  if ischar(cfg.ylim)
    ...
    if strcmp(cfg.ylim,'maxmin')
      ...
      ymin = min([ymin min(datavector-datavector_sem)]);
      ymax = max([ymax max(datavector+datavector_sem)]);
  ...
  if i>1 && strcmp(cfg.maskstyle, 'box')
    ft_plot_vector(xval, datavector, 'ysem', datavector_sem, 'style', cfg.linestyle{i}, 'color', color, ...
