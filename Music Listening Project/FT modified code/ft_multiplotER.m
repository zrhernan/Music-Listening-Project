...
if strcmp(cfg.ylim, 'maxmin') || strcmp(cfg.ylim, 'maxabs')
  ...
  for i=1:length(varargin)
    ...
    if isfield(varargin{i},'var')
      dat_sem = sqrt(varargin{i}.var);
    elseif isfield(varargin{i},'powspctrmsem')
    ...
    data_sem = dat_sem(intersect(seldat1,seldat2),:);
    ymin = min([ymin min(min(min(data-data_sem)))]);
    ymax = max([ymax max(max(max(data+data_sem)))]);
  end
...
ft_plot_text(hlim(1), 0, [num2str(hlim(1), 3) ' '], placement{:}, 'Fontsize', cfg.fontsize, 'VerticalAlignment', 'top');%, 'HorizontalAlignment', 'Right');
ft_plot_text(hlim(2), 0, [num2str(hlim(2), 3) ' '], placement{:}, 'Fontsize', cfg.fontsize, 'VerticalAlignment', 'top');%, 'HorizontalAlignment', 'Right');
ft_plot_text(0, vlim(1), [num2str(vlim(1), 3) ' '], placement{:}, 'Fontsize', cfg.fontsize, 'HorizontalAlignment', 'Right');%, 'VerticalAlignment', 'bottom');
ft_plot_text(0, vlim(2), [num2str(vlim(2), 3) ' '], placement{:}, 'Fontsize', cfg.fontsize, 'HorizontalAlignment', 'Right');%, 'VerticalAlignment', 'bottom');
